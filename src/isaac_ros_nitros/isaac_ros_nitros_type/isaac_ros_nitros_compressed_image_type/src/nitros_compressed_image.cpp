...
void rclcpp::TypeAdapter<nvidia::isaac_ros::nitros::NitrosCompressedImage,
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("NitrosCompressedImage"),
               "[convert_to_ros_message] Conversion started for handle=%ld",
               source.handle);
  ...
  if (!maybe_gxf_tensor) {
    ...
    error_msg << "[convert_to_ros_message] Failed to get tensor component from "
                 "message entity: "
              << GxfResultStr(maybe_gxf_tensor.error());
    RCLCPP_ERROR(rclcpp::get_logger("NitrosCompressedImage"),
                 error_msg.str().c_str());
  ...
  auto maybe_codec = msg_entity->get<int32_t>("codec");
  if (maybe_codec) {
    destination.format = (*maybe_codec.value() == 1) ? "h265" : "h264";
  }
  destination.data.resize(gxf_tensor->size());

  // compressed results are reside on CPU
  const cudaError_t cuda_error =
      cudaMemcpy(destination.data.data(), gxf_tensor->data<uint8_t>().value(),
                 gxf_tensor->size(), cudaMemcpyHostToHost);
...
void rclcpp::TypeAdapter<nvidia::isaac_ros::nitros::NitrosCompressedImage,
                         sensor_msgs::msg::CompressedImage>::
    convert_to_custom(const ros_message_type &source,
  ...
  if (!maybe_allocator_handle) {
    std::stringstream error_msg;
    error_msg << "[convert_to_custom] Failed to get allocator's handle: "
  ...
  auto gxf_tensor_result = gxf_tensor.value()->reshape<uint8_t>(
      ...
      nvidia::gxf::MemoryStorageType::kHost, allocator_handle);
  if (!gxf_tensor_result) {
    std::stringstream error_msg;
    error_msg << "[convert_to_custom] Error Creating tensors: "
  ...
  const cudaError_t cuda_error =
      cudaMemcpy(gxf_tensor.value()->data<uint8_t>().value(),
                 source.data.data(), source.data.size(), cudaMemcpyHostToHost);

  if (cuda_error != cudaSuccess) {
    std::stringstream error_msg;
    error_msg << "[convert_to_custom] cudaMemcpy failed for copying data from "
  ...
  if (!output_timestamp) {
    std::stringstream error_msg;
    error_msg << "[convert_to_custom] Failed to add a timestamp component to "
                 "message: "
