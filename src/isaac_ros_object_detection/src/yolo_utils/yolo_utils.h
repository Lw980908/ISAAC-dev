...
namespace yolo_utils
{
    namespace dataSets
    {
        ...
        // const std::vector<std::string> detect_people = {"people"};
        const std::vector<std::string> universal_detect = {
            "people",
            "bucket",
            "tooth",
            "root",
    ...
    namespace Colors
    {
        ...
        //     cv::Scalar(113, 123, 232), cv::Scalar(69, 205, 80), cv::Scalar(18, 170, 49), cv::Scalar(89, 51, 241), cv::Scalar(153, 191, 154), cv::Scalar(27, 26, 69),
        //     cv::Scalar(20, 186, 194), cv::Scalar(210, 202, 167), cv::Scalar(196, 113, 204), cv::Scalar(9, 81, 88), cv::Scalar(191, 162, 67), cv::Scalar(227, 73, 120)};
        // const std::vector<cv::Scalar> color_people{
        //     cv::Scalar(0, 0, 255),
        // };
        const std::vector<cv::Scalar> colors_universal = {
            cv::Scalar(0, 0, 255),
            cv::Scalar(255, 0, 0),
            cv::Scalar(80, 255, 127),
            cv::Scalar(0, 0, 255),
            cv::Scalar(0, 165, 255),
            cv::Scalar(0, 165, 255),
            cv::Scalar(0, 165, 255),
            ...
            cv::Scalar(0, 165, 255),
            cv::Scalar(0, 165, 255),
            cv::Scalar(128, 114, 246),
            cv::Scalar(149, 177, 248),
            cv::Scalar(116, 85, 176)};
    }
    ...
    //     size_t batch_size;
    //     int teeth_complete_num;
    //     bool has_root_when_missing;
    //     bool use_classifier;
    //     int src_h, src_w;
    //     int dst_h, dst_w;
    ...
    struct CandidateObject
    {
        ...
        CandidateObject(float *bboxAndkeyPoints, float score, bool isGood) : mScore(score),
                                                                             mIsGood(isGood)
        {
    ...
    //     Box(float left, float top, float right, float bottom, float confidence, int label, int numLandMarks) : left(left), top(top), right(right), bottom(bottom), confidence(confidence), label(label)
    //     {
    ...
    // void setInputStream(ImageProcessingConfig &image_processing_config, const InputStream &source, const std::string &imagePath,
    //                     const std::string &videoPath, const int &cameraID, cv::VideoCapture &capture, InitParameter &param);
    ...
    //           const int &cvDelayTime, std::vector<cv::Mat> &imgsBatch, std::unordered_set<int> &tooth_state,
    //           std::unordered_set<int> &teeth_and_root, std::vector<int> &roi_region,
    //           std::string &show_tooth_state_name, int &show_people_cnt, const std::string &windows_title, cv::Mat &compositeImage, yolo_utils::InputStream source);

    // // cv::Mat draw_detection_boxes(const std::vector<std::vector<yolo_utils::Box>> &objectss, const std::vector<std::string> &classNames, cv::Mat &img,
    // //                              ToothDetectionConfig &tooth_detection_config, PedestrianDetectionConfig &pedestrian_detection_config, std::vector<int> &roi_region);
    ...
    //           const int &batchi, std::unordered_set<int> &tooth_state, std::unordered_set<int> &teeth_and_root, std::vector<int> &roi_region);
