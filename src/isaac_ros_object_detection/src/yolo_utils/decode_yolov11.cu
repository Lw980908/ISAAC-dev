...
__global__ void decode_yolov11_device_kernel(int batch_size, int  num_class, int topK, float conf_thresh,
...
void yolov11::decodeDevice(InitParameter param, float* src, int srcWidth, int srcHeight, int srcArea, float* dst, int dstWidth, int dstHeight, cudaStream_t stream)
{
	...
	decode_yolov11_device_kernel << < grid_size, block_size, 0, stream >> > (param.batch_size, param.num_class, param.topK, param.conf_thresh,
...
void yolov11::transposeDevice(InitParameter param, 
float* src, int srcWidth, int srcHeight, int srcArea, 
float* dst, int dstWidth, int dstHeight, cudaStream_t stream)
{
	...
	transpose_device_kernel << < grid_size, block_size, 0, stream >> > (param.batch_size,
		src, srcWidth, srcHeight, srcArea,
		dst, dstWidth, dstHeight, dstArea);
}
