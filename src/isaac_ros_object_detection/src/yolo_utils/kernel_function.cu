...
__device__ 
void affine_project_device_kernel(yolo_utils::AffineMat* matrix, int x, int y, float* proj_x, float* proj_y)
{
...
void resize_rgb_padding_device_kernel(float* src, int src_width, int src_height, int src_area, int src_volume,
	...
	int batch_size, float padding_value, yolo_utils::AffineMat matrix)
{
...
void resize_rgb_without_padding_device_kernel(float* src, int src_width, int src_height, int src_area, int src_volume,
	float* dst, int dst_width, int dst_height, int dst_area, int dst_volume,
	int batch_size, yolo_utils::AffineMat matrix)
{
	int dx = blockDim.x * blockIdx.x + threadIdx.x;
	int dy = blockDim.y * blockIdx.y + threadIdx.y;
	if (dx < dst_area && dy < batch_size)
...
void resizeDevice(const int& batchSize, float* src, int srcWidth, int srcHeight,
	...
	float paddingValue, yolo_utils::AffineMat matrix, cudaStream_t stream)
{
	...
	resize_rgb_padding_device_kernel << < grid_size, block_size, 0, stream >> > (src, srcWidth, srcHeight, src_area, src_volume,
...
void resizeDevice(const int& batchSize, float* src, int srcWidth, int srcHeight,
	float* dst, int dstWidth, int dstHeight, 
	yolo_utils::ColorMode mode, yolo_utils::AffineMat matrix, cudaStream_t stream)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((dstWidth * dstHeight + BLOCK_SIZE - 1) / BLOCK_SIZE,
		(batchSize + BLOCK_SIZE - 1) / BLOCK_SIZE);
	...
	switch (mode)
	{
	case yolo_utils::ColorMode::RGB:
		resize_rgb_without_padding_device_kernel << < grid_size, block_size, 0, stream >> > (src, srcWidth, srcHeight, src_area, src_volume,
			dst, dstWidth, dstHeight, dst_area, dst_volume,
			batchSize, matrix);
		return;
	case yolo_utils::ColorMode::GRAY:
		resize_gray_without_padding_device_kernel << < grid_size, block_size, 0, stream >> > (src, srcWidth, srcHeight, src_area, 
...
void normDevice(const int& batchSize, float* src, int srcWidth, int srcHeight,
	float* dst, int dstWidth, int dstHeight,
	InitParameter param, cudaStream_t stream)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((dstWidth * dstHeight * 3 + BLOCK_SIZE - 1) / BLOCK_SIZE,
		(batchSize + BLOCK_SIZE - 1) / BLOCK_SIZE);
	...
	int img_height = srcHeight;
	int img_width = srcWidth;
	norm_device_kernel << < grid_size, block_size, 0, stream >> > (src, dst, batchSize, img_height, img_width, img_area, img_volume,
...
void decodeDevice(InitParameter param, float* src, int srcWidth, int srcHeight, int srcArea, float* dst, int dstWidth, int dstHeight, cudaStream_t stream)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	...
	decode_yolo_device_kernel << < grid_size, block_size, 0, stream >> > (param.batch_size, param.num_class, param.topK, param.conf_thresh,
		src, srcWidth, srcHeight, srcArea,
		dst, dstWidth, dstHeight, dstArea);
}

void nmsDeviceV1(InitParameter param, float* src, int srcWidth, int srcHeight, int srcArea, cudaStream_t stream)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((param.topK + BLOCK_SIZE - 1) / BLOCK_SIZE, 
		(param.batch_size + BLOCK_SIZE - 1) / BLOCK_SIZE);

	nms_fast_kernel << < grid_size, block_size, 0, stream >> > (param.topK, param.batch_size, param.iou_thresh,
		src, srcWidth, srcHeight, srcArea);
}

void nmsDeviceV2(InitParameter param, float* src, int srcWidth, int srcHeight, int srcArea, 
	int* idx, float* conf, cudaStream_t stream)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((param.topK + BLOCK_SIZE - 1) / BLOCK_SIZE, 
		(param.batch_size + BLOCK_SIZE - 1) / BLOCK_SIZE);
	get_key_val_kernel << < grid_size, block_size, 0, stream >> > (param.batch_size, src, srcWidth, srcHeight, srcArea, idx, conf);
	for (size_t i = 0; i < param.batch_size; i++)
	{
		int* p_idx     = idx + i * srcHeight;
		float* p_conf = conf + i * srcHeight;
		thrust::sort_by_key(thrust::cuda::par.on(stream), p_conf, p_conf + srcHeight, p_idx, thrust::greater<float>());
	}
	nms_sort_kernel << < grid_size, block_size, 0, stream >> > (param.topK, param.batch_size, param.iou_thresh,
		src, srcWidth, srcHeight, srcArea, idx);
}

void copyWithPaddingDevice(const int& batchSize, float* src, int srcWidth, int srcHeight,
	float* dst, int dstWidth, int dstHeight, float paddingValue, int padTop, int padLeft, cudaStream_t stream)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((dstWidth * dstHeight + BLOCK_SIZE - 1) / BLOCK_SIZE,
		(batchSize + BLOCK_SIZE - 1) / BLOCK_SIZE);
	...
	copy_with_padding_kernel_function << < grid_size, block_size, 0, stream >> > (batchSize, src, srcWidth, srcHeight, src_area, src_volume,
