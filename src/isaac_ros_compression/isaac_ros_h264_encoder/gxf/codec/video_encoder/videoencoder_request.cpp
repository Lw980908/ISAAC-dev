...
gxf_result_t VideoEncoderRequest::registerInterface(gxf::Registrar *registrar) {
  ...
  result &= registrar->parameter(
      inbuf_storage_type_, "inbuf_storage_type",
      "Input Buffer storage(memory) type",
      "Input Buffer storage type, 0:host mem, 1:device mem", 1u);
  result &=
      registrar->parameter(videoencoder_context_, "videoencoder_context",
                           "videoencoder context", "Encoder context Handle");
  // Encoder related parameters
  result &= registrar->parameter(codec_, "codec", "Video Codec to use",
                                 "Video codec,  0:H264, 1:H265", 0);
  ...
  result &= registrar->parameter(input_format_, "input_format",
                                 ...
                                 "nv12", gxf::EncoderInputFormat::kNV12);
  result &= registrar->parameter(profile_, "profile", "Encode profile",
                                 "0:Baseline Profile, 1: Main , 2: High", 2);
  ...
  result &= registrar->parameter(
      ...
      "Maximum data rate and resolution, select from 0 to 14", 14);
  result &= registrar->parameter(iframe_interval_, "iframe_interval",
                                 "I Frame Interval",
                                 "Interval between two I frames", 30);
  result &=
      registrar->parameter(rate_control_mode_, "rate_control_mode",
                           "Rate control mode, 0:CQP[RC off], 1:CBR, 2:VBR ",
                           "Rate control mode", 1);
  result &= registrar->parameter(qp_, "qp", "Encoder constant QP value",
                                 "cont qp", 20u);
  result &= registrar->parameter(
      bitrate_, "bitrate", "Encoder bitrate",
      "Bitrate of the encoded stream, in bits per second", 20000000);
  ...
  result &= registrar->parameter(
      ...
      "Preset of config", gxf::EncoderConfig::kCustom);
...
gxf_result_t VideoEncoderRequest::deinitialize() { return GXF_SUCCESS; }
...
gxf_result_t VideoEncoderRequest::start() {
  ...
  if (impl_->ctx->is_cuvid) {
    if (impl_->ctx->rate_control_mode == 0) {
      CHECK_ENCODER_ERROR(
          setRateControlMode(impl_->ctx, V4L2_MPEG_VIDEO_BITRATE_MODE_CONSTQP),
          "Failed to set Rate control mode")
    } else {
      CHECK_ENCODER_ERROR(
          setRateControlMode(impl_->ctx, V4L2_MPEG_VIDEO_BITRATE_MODE_CBR),
          "Failed to set Rate control mode")
    }
...
gxf_result_t VideoEncoderRequest::stop() { return GXF_SUCCESS; }
gxf_result_t
VideoEncoderRequest::prepareOutputEntity(const gxf::Entity &input_msg) {
  ...
  if (maybe_input_intrinsics) {
    auto output_intrinsics =
        output_entity->add<gxf::CameraModel>(maybe_input_intrinsics->name());
  ...
  if (maybe_input_extrinsics) {
    auto output_extrinsics =
        output_entity->add<gxf::Pose3D>(maybe_input_extrinsics->name());
  ...
  if (maybe_input_seq_number) {
    auto output_seq_number =
        output_entity->add<int64_t>(maybe_input_seq_number->name());
  ...
  if (maybe_gxf_timestamp) {
    auto output_gxf_timestamp =
        output_entity->add<gxf::Timestamp>(kTimestampName);
  ...
  auto output_tensor =
      output_entity->add<nvidia::gxf::Tensor>(maybe_input_image->name());
  if (!output_tensor) {
    GXF_LOG_ERROR("Failed to add output tensor");
    return gxf::ToResultCode(output_tensor);
  }

  auto output_codec = output_entity->add<int32_t>("codec");
  if (!output_codec) {
    GXF_LOG_ERROR("Failed to add codec into output");
    return gxf::ToResultCode(output_codec);
  }
  *output_codec.value() = codec_;
...
gxf_result_t VideoEncoderRequest::queueInputYUVBuf(
  ...
  NvBufSurface *nvbuffer0 = reinterpret_cast<NvBufSurface *>(
      impl_->ctx->output_buffers[q_index].buf_surface);
  if (impl_->ctx->is_cuvid) {
    pdst_data = reinterpret_cast<uint8_t *>(nvbuffer0->surfaceList[0].dataPtr);
  } else {
    result = cudaHostGetDevicePointer(
        &pdst_data, nvbuffer0->surfaceList[0].mappedAddr.addr[0], 0);
  ...
  uint32_t plane = 0;
  result = cudaMemcpy2D(
      pdst_data, nvbuffer0->surfaceList[plane].pitch, input_img->pointer(),
  ...
  if (impl_->ctx->is_cuvid) {
    ...
    result = cudaHostGetDevicePointer(
        &pdst_data, nvbuffer0->surfaceList[0].mappedAddr.addr[plane], 0);
    if (result != cudaSuccess) {
      GXF_LOG_ERROR("Failed in cudaHostGetDevicePointer: %s",
                    cudaGetErrorString(result));
      return GXF_FAILURE;
  ...
  if (input_format_.get() == gxf::EncoderInputFormat::kYUV420PLANAR) {
    ...
    result =
        cudaMemcpy2D((unsigned char *)pdst_data,
                     nvbuffer0->surfaceList[0].planeParams.pitch[plane],
                     input_img->pointer() + src_offset,
                     input_img_info.color_planes[plane].stride,
                     (input_img_info.color_planes[plane].width *
...
gxf_result_t VideoEncoderRequest::checkInputParams() {
  ...
  if (input_width_ % 2 != 0) {
    GXF_LOG_ERROR(
        "Error in input parameter: input_width must be an even number");
    return GXF_FAILURE;
  }
  ...
  if (input_height_ % 2 != 0) {
    GXF_LOG_ERROR(
        "Error in input parameter: input_height must be an even number");
    return GXF_FAILURE;
  }
...
gxf_result_t VideoEncoderRequest::getInputFormatFromParameter() {
...
gxf_result_t VideoEncoderRequest::getProfileLevelSettingsFromParameter() {
  if (codec_ == 1) {
    switch (profile_) {
    case 0:
      impl_->ctx->profile = V4L2_MPEG_VIDEO_H265_PROFILE_MAIN;
      break;
    case 1:
      impl_->ctx->profile = V4L2_MPEG_VIDEO_H265_PROFILE_MAIN10;
      break;
    default:
    ...
    switch (level_) {
    case 0:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_1_0_MAIN_TIER;
      break;
    case 1:
    case 2:
    case 3:
    case 4:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_2_0_MAIN_TIER;
      break;
    case 5:
    case 6:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_2_1_MAIN_TIER;
      break;
    case 7:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_3_0_MAIN_TIER;
      break;
    case 8:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_3_1_MAIN_TIER;
      break;
    case 9:
    case 10:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_4_0_MAIN_TIER;
      break;
    case 11:
    case 12:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_4_1_MAIN_TIER;
      break;
    case 13:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_5_0_MAIN_TIER;
      break;
    case 14:
      impl_->ctx->level = V4L2_MPEG_VIDEO_H265_LEVEL_5_1_MAIN_TIER;
      break;
    default:
      GXF_LOG_ERROR("The Video H265 level is not supported");
      return GXF_FAILURE;
    }
  } else {
    switch (profile_) {
...
gxf_result_t VideoEncoderRequest::getEncoderSettingsFromParameters() {
  ...
  impl_->ctx->encoder_pixfmt =
      (codec_ == 1) ? V4L2_PIX_FMT_H265 : V4L2_PIX_FMT_H264;
  ...
  if (config_.get() == gxf::EncoderConfig::kPFrameCQP) {
    impl_->ctx->qp = 20;
    impl_->ctx->profile = (codec_ == 1) ? V4L2_MPEG_VIDEO_H265_PROFILE_MAIN
...
gxf_result_t VideoEncoderRequest::setEncoderParameters() {
  ...
  if (impl_->ctx->is_cuvid) {
    ...
    if (impl_->ctx->rate_control_mode == 0) {
      CHECK_ENCODER_ERROR(setConstantQP(impl_->ctx, impl_->ctx->qp,
                                        impl_->ctx->qp, impl_->ctx->qp),
                          "Failed to set Const QP value")
    } else {
    ...
    if (impl_->ctx->rate_control_mode == 0) {
      ...
      CHECK_ENCODER_ERROR(
          setInitQP(impl_->ctx, impl_->ctx->qp, impl_->ctx->qp, impl_->ctx->qp),
      ...
      if (impl_->ctx->rate_control_mode == 2) {
        CHECK_ENCODER_ERROR(
            setRateControlMode(impl_->ctx, V4L2_MPEG_VIDEO_BITRATE_MODE_VBR),
    ...
    CHECK_ENCODER_ERROR(
        setIFrameInterval(impl_->ctx, impl_->ctx->iframe_interval),
        "Failed to set IFrame Interval")
